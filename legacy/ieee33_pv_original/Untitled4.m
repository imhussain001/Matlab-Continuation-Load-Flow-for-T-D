...
VoltageAtBuses = []; % Store voltage magnitudes for buses 30, 31, 32, and 33
...
while lambda <= maxLambda
    ...
    % Store results for specified buses
    VoltageAtBuses = [VoltageAtBuses; abs(v([30, 31, 32, 33]))']; % Store voltage magnitudes
    LambdaValues = [LambdaValues; lambda]; % Scaling factor for load

    lambda = lambda + sigma; % Increase lambda for next iteration
end

% Plot Voltage Profiles for Buses 30, 31, 32, and 33
figure;
plot(LambdaValues, VoltageAtBuses(:, 1), 'b-', 'LineWidth', 1.5, 'DisplayName', 'Bus 30');
hold on;
plot(LambdaValues, VoltageAtBuses(:, 2), 'r-', 'LineWidth', 1.5, 'DisplayName', 'Bus 31');
plot(LambdaValues, VoltageAtBuses(:, 3), 'g-', 'LineWidth', 1.5, 'DisplayName', 'Bus 32');
plot(LambdaValues, VoltageAtBuses(:, 4), 'm-', 'LineWidth', 1.5, 'DisplayName', 'Bus 33');
xlabel('Lambda (Load Scaling Factor)');
ylabel('Voltage Magnitude (pu)');
title('Voltage Profiles for Buses 30, 31, 32, and 33');
grid on;
