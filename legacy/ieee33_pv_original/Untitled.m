...
% Modify reactive power at Bus 33 for analysis
bus(33, 3) = 0.9; % Set a larger reactive power for Bus 33 in MW
...
while lambda <= maxLambda
    % Scale only reactive power (ignore real power scaling)
    scaleFactorQ = 1.5; % Amplify reactive power
    P(:,1) = lambda * complex(bus(:,2), bus(:,3) * scaleFactorQ);
    ...
    for iter = 1:maxIterations
        ...
        v1 = DLF(2:bn, 2:bn) * I1; % Voltage drop along branches
    ...
    lambda = lambda + sigma; % Increase lambda
end

% Plot Loadability Curve
figure;
plot(LambdaValues, VoltageAtBus, 'go-', 'LineWidth', 1.5, 'MarkerSize', 6);
